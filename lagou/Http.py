...
class Http:
    ...
    def get(self, url, headers=None, cookies=None, proxy=None, time_out=5, time_out_retry=3):
        '''
        获取网页源码
        url: 网页链接
        headers: headers
        ...
        try:
            ...
            response = requests.get(url, headers=headers, cookies=cookies, proxies=proxy, timeout=time_out)
            ...
            if time_out_retry > 0:
                htmlCode = self.get(url=url, timeOutRetry=(time_out_retry - 1))
            else:
                logging.error('GetTimeOut %s' % url)
                htmlCode = 'None'
        return htmlCode

    def post(self, url, param, headers=None, cookies=None, proxy=None, time_out=5, time_out_retry=3):
        '''
        post获取响应
        url: 目标链接
        param: 参数
        ...
        try:
            ...
            response = requests.post(url, data=param, headers=headers, cookies=cookies, proxies=proxy, timeout=time_out)
            if response.status_code == 200 or response.status_code == 302:
                htmlCode = response.text
            else:
                htmlCode = None
            logging.info('Post %s %s' % (str(response.status_code), url))
        except Exception as e:
            logging.error('PostExcept %s' % str(e))
            if time_out_retry > 0:
                htmlCode = self.post(url=url, param=param, time_out_retry=(time_out_retry - 1))
            else:
